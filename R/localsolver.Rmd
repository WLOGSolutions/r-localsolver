...
__`LocalSolver` is required by the package.__  It can be obtained from `http://www.localsolver.com/download.html`. 
At least trial version of `LocalSolver` must be available and it is sufficient for simple problems. Some demos require licensed 
`LocalSolver`. The package have been tested with version 4.0 of `LocalSolver`.
...
This function requires at least one argument - `model.text.lsp` - a string with model formulation in the special programming language, connected directly with the `LocalSolver` software. For more details, please visit the web site: `http://www.localsolver.com/`.

The model description in the `LocalSolver` language should include the declaration of decision variables, the formulation of the constraints (in terms of model parameters) and the objective functions to be minimized or maximized. All the objective functions or constraints whose values we would like to extract from the problem must be named.
...
will not change the `lsp` object, whereas
...
If the function is called many times on the same object, the settings are updated (the values parameters chosen, when the function is called once again, will be changed, and the parameters whose values were set before, will not be changed if they are not called once again). 
...
It is also possible to reset the parameters:
...
The particular demos may be activated by calling the same command and specifying the demo name, for instance:

```
demo(assignment, package='localsolver')
```

Note that some of the demo problems have more than 1000 decision variables and therefore demand the full license for the 'LocalSolver' program.
